// from https://github.com/datafuselabs/openraft/blob/v0.7.0-alpha.1/memstore/src/lib.rs
...
impl RaftSnapshotBuilder<Config, Cursor<Vec<u8>>> for Arc<MemStore> {
    ...
    async fn build_snapshot(
        &mut self,
    ) -> Result<Snapshot<MemNodeId, Cursor<Vec<u8>>>, StorageError<MemNodeId>> {
        let data;
...

        {
            ...
            data = serde_json::to_vec(&*sm).map_err(|e| {
                StorageIOError::new(
...
impl RaftStorage<Config> for Arc<MemStore> {
    ...
    async fn begin_receiving_snapshot(
        &mut self,
    ) -> Result<Box<Self::SnapshotData>, StorageError<MemNodeId>> {
